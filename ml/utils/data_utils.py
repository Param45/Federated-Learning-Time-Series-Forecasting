...
def generate_time_lags(
    df: pd.DataFrame, n_lags: int = 10, identifier: str = "District", is_y: bool = False
) -> pd.DataFrame:
        ...
        for n in range(1, n_lags + 1):
            for col in columns:
                if col == "time" or col == identifier:
                    continue
                df_n[f"{col}_lag-{n}"] = (
                    df_n[col].shift(n).replace(np.nan, 0).astype("float64")
                )
...
def time_to_feature(
    df: pd.DataFrame, use_time_features: bool = True, identifier: str = "District"
    ...
    df = (
        ...
        .assign(hour=df.index.hour).assign(  # hour of day
...
def to_cyclical(
    df: pd.DataFrame, col_names_period: Dict[str, int], start_num: int = 0
) -> pd.DataFrame:
...
def to_timeseries_rep(
    x: Union[np.ndarray, Dict[Union[str, int], np.ndarray]],
    num_lags: int = 10,
    num_features: int = 11,
...
def scale_features(
    ...
    per_area: bool = False,
    identifier: str = "District",
) -> Union[pd.DataFrame, pd.DataFrame, Any]:
...
def to_train_val(
    df: pd.DataFrame, train_size: float = 0.8, identifier: str = "District"
) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if df[identifier].nunique() != 1:
        ...
        for area in df[identifier].unique():
            ...
            log(
                INFO,
                f"\tNumber of samples for validation:  {len(area_data) - num_samples_train}",
            )
...
def to_Xy(
    ...
    if (
        ignore_cols is not None
        and len(ignore_cols) > 0
        and next(iter(ignore_cols)) is not None
    ):
...
def get_exogenous_data_by_area(
    exogenous_data_train: pd.DataFrame,
    ...
    if exogenous_data_val is not None:
        assert list(exogenous_data_val[identifier].unique()) == list(
            exogenous_data_train[identifier].unique()
        )
    area_exogenous_data_train, area_exogenous_data_val = dict(), dict()
    for area in exogenous_data_train[identifier].unique():
        # get per area observations
        exogenous_data_train_area = exogenous_data_train.loc[
            exogenous_data_train[identifier] == area
        ]
        area_exogenous_data_train[area]: pd.DataFrame = exogenous_data_train_area
        if exogenous_data_val is not None:
            exogenous_data_val_area = exogenous_data_val.loc[
                exogenous_data_val[identifier] == area
            ]
    ...
    for area in area_exogenous_data_train:
        area_exogenous_data_train[area] = area_exogenous_data_train[area].drop(
            [identifier], axis=1
        )
        if exogenous_data_val is not None:
            area_exogenous_data_val[area] = area_exogenous_data_val[area].drop(
                [identifier], axis=1
            )
...
def to_torch_dataset(
    ...
    tensor_dataset = TimeSeriesDataset(
        X, y, num_lags, num_features, indices, exogenous_data
    )
...
class TimeSeriesDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        if exogenous is not None:
            assert X.size(0) == y.size(0) == exogenous.size(0), (
